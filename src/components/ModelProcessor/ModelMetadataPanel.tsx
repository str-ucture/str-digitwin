...
function Row({ label, value, highlight }: { label: string; value: React.ReactNode; highlight?: boolean }) {
  return (
    <div className={`flex items-start justify-between gap-4 py-1.5 border-b border-gray-50 last:border-0 ${
      ...
      <span className="text-xs font-medium text-right font-mono">{value}</span>
    </div>
  )
}
...
export default function ModelMetadataPanel({ file, scene, outputBytes, inputBytes, processingDesc, loading }: Props) {
  ...
  return (
    <div className={`rounded-xl border border-gray-200 bg-white p-4 flex flex-col ${isCollapsed ? 'flex-shrink-0' : 'flex-1 min-h-0'}`}>
      <div
      ...
      >
        <div className="flex items-center gap-2">
          ...
          <span className="text-sm font-semibold text-gray-700">File Metadata</span>
        </div>
        <button
          type="button"
